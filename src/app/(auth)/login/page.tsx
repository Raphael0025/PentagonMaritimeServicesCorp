...
export default function Login() {
    ...
    const handleSubmit = async () => {
        try{
            ...
            router.push('/enterprise-portal/admin')
        ...
        <main className='w-full h-dvh flex bg-wave' style={{backgroundColor: '#E6E6E6'}}>
            <section className='login-container py-5 px-20 flex flex-col justify-center items-center'>
                <section className='space-y-8'>
                    ...
                    <section className='flex flex-col justify-center items-center w-full space-y-8'>
                        <Box className='input-grp' w='75%'>
                            <FormControl isRequired isInvalid={touched.user_code && !state.user_code}>
                                <Input id='user_code' fontSize='md' size='lg' placeholder='' type='text' name='user_code' value={state.user_code} onBlur={onBlur} onChange={handleChange} className='p-3 form-input' border='0' borderBottom='2px' borderColor='#A1A1A1' borderRadius='0' errorBorderColor='red.300' autoComplete='off' _hover={{borderColor: '#2F67B2'}} _focus={{ boxShadow:'0 0 0px 0px rgba(88, 144, 255, .75), 0 0px 0px rgba(0, 0, 0, .15)'}}/>
                                <label htmlFor='user_code' className='form-label-login'>User Code </label>
                                <FormErrorMessage w='100%' fontSize='xs' className='flex justify-end font-normal'>User Code required</FormErrorMessage>
                            </FormControl>
                        </Box>
                        <Box className='input-grp' w='75%'>
                            <FormControl isRequired isInvalid={touched.password && !state.password}>
                                <InputGroup>
                                    <Input id='password' fontSize='md' size='lg' placeholder='' name='password' value={state.password} onBlur={onBlur} onChange={handleChange} type={show ? 'text' : 'password'} className='p-3  form-input' border='0' borderBottom='2px' borderColor='#A1A1A1' borderRadius='0' errorBorderColor='red.300' autoComplete='off' _hover={{borderColor: '#2F67B2'}} _focus={{ boxShadow:'0 0 0px 0px rgba(88, 144, 255, .75), 0 0px 0px rgba(0, 0, 0, .15)'}}/>
                                    <label htmlFor='password' className='form-label-login'>Password </label>
                                    <InputRightElement>
                                        <button onClick={() => {setShow(!show)}}>
                                        {show ? <Image src='/icons/hide.png' className='me-2' alt='next-arrow' width={24} height={24}/> 
                                        : <Image src='/icons/show.png' className='me-2' alt='next-arrow' width={24} height={24}/>}
                                        </button>
                                    </InputRightElement>
                                </InputGroup>
                                <FormErrorMessage w='100%' fontSize='xs' className='flex justify-end font-normal'>Password required</FormErrorMessage>
                            </FormControl>
                        </Box>
