...
interface RegistrationContextType {
    ...
    allData: REGISTRATION_BY_ID[] | null;
...
const RegistrationContext = createContext<RegistrationContextType>({data: null, allData: null, lastMonthReg: null, setRegistrations: () => {}, setMonth: () => {}, setYear: () => {}})
...
export const RegistrationProvider: React.FC<RegistrationProvderProps>= ({ children }) => {
    ...
    const [allData, setAllData] = useState<REGISTRATION_BY_ID[] | null>(null)
    ...
    useEffect(() => {
        // const fetchData = async () => {
            try{
                ...
                const allRegQuery = query(registrationRef)
                ...
                const unsubscribeAllData = onSnapshot(allRegQuery, (snapshot) => {
                    const updatedData = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as REGISTRATION_BY_ID[];
    ...
    return (
        <RegistrationContext.Provider value={{data, allData, lastMonthReg, setRegistrations: setData, setMonth, setYear}}>
            { children }
