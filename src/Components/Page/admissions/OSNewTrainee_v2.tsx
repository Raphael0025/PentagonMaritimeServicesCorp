...
export default function OSNewTrainee_v2(){
    ...
    const handleSubmit = async () => {
        try{
            ...
            const accountTypes = [
                { type: 1, courses: courses.filter(c => c.accountType !== 0)},
                { type: 0, courses: courses.filter(c => c.accountType === 0)},
            ]
            
            for(const chargeType of accountTypes){
                if(chargeType.courses.length === 0) continue

                const totalFee = chargeType.courses.reduce((sum, c) => sum + (c.course_fee || 0), 0)

                const registrationID = await addRegistrationDetails(traineeID, totalFee, 1,0,chargeType.type, trainee.marketing)
                
                if(registrationID){
                    await Promise.all(
                        chargeType.courses.map(course =>
