...
export default function NewRegistrationForm({ onStepChange = () => {} }: Props){
    ...
    const handleNextStep = (step: string) => {
        scrollToTop(step)
        if(step === 'step1'){
            if(trainee.last_name === '' || trainee.first_name === '' || trainee.rank === '' || trainee.srn === '' || trainee.birthPlace === '' || trainee.contact_no === '' || trainee.email === '' || trainee.nationality === '' || trainee.gender === '' || trainee.endorser === '' || trainee.company === '' || trainee.vessel === '' || trainee.e_contact === '' || trainee.e_contact_person === '' || trainee.relationship === '' || ((trainee.house_no === '' || trainee.street === '' || trainee.brgy === '' || trainee.city === '' || trainee.area === '') && (trainee.otherAddress === '')) || valid === '' || validProfile === '' || preview === null){
                setShowAlert1(true)
            } else {   
                setShowAlert1(false)
                onStepChange(1); 
    ...
    const handleSubmit = async () => {
        try{    
            ...
            const traineeID = {}
            //await addNewTrainee( trainee, traineeType, validID, validPfp, validSignature, file, pfpFile)
            ...
            if(traineeID !== null){
                ...
                if(ccArr.length !== 0){
                    ...
                    regCCID = {}
                    //await addRegistrationDetails(traineeID, fee, regApproach, traineeType, 1, '')
                    for(const course of ccArr){
                        try{
                            if(regCCID){
                ...
                if(crewArr.length !== 0){
                    ...
                    regCrewID = {}
                    //await addRegistrationDetails(traineeID, fee, regApproach, traineeType, 0, '')
                    for(const course of crewArr){
                        try{
                            if(regCrewID){
    ...
    <Box >
        <Box maxH={{base: '580px', md: `100%`}} overflowY={'auto'}>
            <Box className={`${show === 'info' ? '' : 'hidden'} rounded border outline-0 uppercase shadow-md space-y-8 p-7 animate__animated animate__fadeInRight`}>
                            ...
                            <AlertDescription className='flex w-full'>
                                <Box className='w-full flex'>
                                    <Box className='flex flex-col w-full'>
                                        ...
                                        <p>{valid === '' ? `* Valid ID.` : ''}</p>
                                        <p>{validProfile === '' ? `* 2x2 Photo.` : ''}</p>
                                        <p>{preview === null ? `* Signature.` : ''}</p>
                                    </Box>
                                </Box>
                            </AlertDescription>
                        </Box>
                ...
                <Box ref={trainSectionRef} className='flex flex-col items-center w-full md:w-auto'>
                ...
                <Box className='space-y-6'>
                    ...
                    <Box display='flex' gridGap={4} flexDir={{md:'row', base:'column'}} >
                        <FormControl isInvalid={trainee.gender === '' && showAlert1}>
                            ...
                            <Select id='gender' onChange={handleSelect} className='uppercase shadow-md'>
                    ...
                    <Box className='flex justify-end'>
                        <Button className='w-full md:w-1/4' onClick={() => { handleNextStep('step1'); handleCombineBirthDate();}} bgColor='blue.700' colorScheme='blue'>Proceed to Next Step</Button>
                    </Box>
                </Box>
            </Box>
        </Box>
        ...
        <Box maxH={{base: '580px', md: `100%`}} overflowY={'auto'}>
            <Box className={`${show === 'review' ? '' : 'hidden'} rounded border outline-0 uppercase shadow-md space-y-6 p-7 animate__animated animate__fadeInRight`}>
                ...
                <Box className='flex md:flex-row flex-col space-y-4 md:space-y-0  md:justify-between'>
                    <Button onClick={() => { handleNextStep('step1'); onStepChange(2); setShow('train'); }} className='shadow-md uppercase' colorScheme='gray'>Back</Button>
    ...
    <Modal isOpen={isOpenAddress} onClose={onCloseAddress} size='xl' scrollBehavior='inside' motionPreset='slideInRight' >
        <ModalOverlay/>
        <ModalContent className='px-3'>
                                            ...
                                            <AccordionPanel className="space-y-3">
                                                ...
                                                <Box className="flex w-full">
                                                    {courses.some((temp) => temp.course === course.id) ? (
                                                        ...
                                                        <Button isDisabled={courses.some((temp) => temp.course === course.id) || (payment === 3 || sched === '')} colorScheme="blue" bgColor='blue.700' onClick={() => { handleTempCourses( course.id, course.course_fee, course.numOfDays ); }} size="lg" w='100%' > SELECT COURSE </Button>
                                                    )}
                                                </Box>
                                            </AccordionPanel>
                                        </div>
