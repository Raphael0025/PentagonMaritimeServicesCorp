...
export default function NewRegistrationForm({ onStepChange = () => {} }: Props){
    ...
    <Box>
        <Box maxH={{base: '580px', md: `100%`}} overflowY={'auto'}>
            <Box ref={trainSectionRef} className={`${show === 'info' ? '' : 'hidden'} rounded border outline-0 uppercase shadow-md space-y-8 p-7 animate__animated animate__fadeInRight`}>
                {showAlert1 && (
            ...
            <ModalBody>
                ...
                <Box >
                                            ...
                                            <AccordionPanel className="space-y-3">
                                                ...
                                                <Box className="flex w-full">
                                                    {courses.some((temp) => temp.course === course.id) ? (
                                                        ...
                                                        <Button isDisabled={courses.some((temp) => temp.course === course.id) || (payment === 3 || sched === '')} colorScheme="blue" onClick={() => { handleTempCourses( course.id, course.course_fee, course.numOfDays ); }} size="lg" w='100%' > SELECT COURSE </Button>
                                                    )}
                                                </Box>
                                            </AccordionPanel>
                                        </div>
                    ...
                    <Box className="p-3" >
                        ...
                        <Box>
                            <Accordion allowToggle className="space-y-3">
                            {allCourses && allCourses.filter((course) => course.course_code.toUpperCase().includes(search.toUpperCase()) || course.course_name.toUpperCase().includes(search.toUpperCase())).sort((a,b) => a.course_code.localeCompare(b.course_code)) .sort((a, b) => a.course_code.localeCompare(b.course_code)) .map((course) => (
