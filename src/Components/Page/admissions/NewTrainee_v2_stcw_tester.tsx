...
export default function NewTrainee_v2(){
    ...
    const handleCourseSelection = (val: string, type: string) => {
        setTempCourses((prev) =>
                            ...
                            <Box className="space-y-3">
                                {(() => {
                                    const filteredCourses = allCourses?.filter(
                                        ...
                                        .filter((course) => course.courseType === 0) // Force to filter out IN-HOUSE Courses
                                        .sort((a,b) => a.course_code.localeCompare(b.course_code)) || []

                                    return filteredCourses.length > 0 ?(
                                        filteredCourses.map((course) => (
                                            <Box key={course?.id} onClick={() => { handleCourseSelection(course.id, 'course'); handleCourseSelection(course.course_fee.toString(), 'course_fee'); handleCourseSelection(course.numOfDays.toString(), 'numOfDays'); handleSchedule(course.id); onCloseModal(); }} _hover={{ boxShadow: '4px 4px 8px #3182ce',  cursor: 'pointer'}} textTransform='uppercase' shadow='md' className={`rounded`}>
