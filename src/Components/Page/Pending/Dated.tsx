...
import { Image, Box, Text, Input, Button, InputLeftAddon, Menu, VStack, FormLabel, Select, Switch, MenuList, MenuButton, IconButton, MenuItem, MenuGroup, Grid, GridItem, FormControl, MenuDivider, MenuOptionGroup, MenuItemOption, InputGroup, useToast, Accordion, AccordionButton, AccordionPanel, AccordionIcon, AccordionItem, Modal, ModalOverlay, ModalHeader, ModalBody, ModalContent, ModalFooter, ModalCloseButton, useDisclosure } from '@chakra-ui/react';
...
import { ACKNOWLEDGE_REGISTRATION, ENROLL_COURSE, changeImg, updateTraineeAttachments, UPDATE_TRAINING, UPDATE_TRAINEE } from '@/lib/trainee_controller'
...
export default function Page() {
    ...
    const { isOpen: isOpenTrainingDate, onOpen: onOpenTrainingDate, onClose: onCloseTrainingDate } = useDisclosure()
    ...
    const handleInsertActualDates = async (act_start_date: string, act_end_date: string, training_id: string, reg_id: string, trainee_id: string, reg_type: number, reg_account_type: number) => {
        setLoading(true)
        const actor: string | null = localStorage.getItem('customToken')
        const actor_user_code: string | null = localStorage.getItem('userCode')
        ...
        new Promise<void>((res,rej) => {
            setTimeout(async () => {
                try{
                    let batch: string = '1'
                    await UPDATE_TRAINING(training_id, {act_start_date, act_end_date}, actor)
                    await ENROLL_COURSE(Number(actor_user_code), batch, training_id, reg_id, trainee_id, reg_type, reg_account_type, actor)
                    res()
                }catch(error){
                    rej(error)
            ...
            handleToast('Actual Date Inserted Successfully!', `This record has been updated successfully.`, 5000, 'success')
        }).catch((error) => {
            console.log('Error: ', error)
        }).finally(() => {
            setLoading(false)
    ...
    const handleNonAppearance = async (training_id: string, reg_id: string, trainee_id: string, reg_type: number, reg_account_type: number) => {
        setLoading(true);
        ...
        try {
            // Optional delay if you want artificial UI loading time
            await new Promise((res) => setTimeout(res, 1500));

            // 1. Fully completes before moving to step 2
            await ENROLL_COURSE(
                Number(actor_user_code),
                '1',
                training_id,
                reg_id,
                trainee_id,
            ...
            handleToast(
                ...
                'This record has been updated to Non-Appearance.',
                5000,
                'success'
            );
        } catch (error) {
    ...
    return (
        ...
        <Modal isOpen={isOpenScope} onClose={onCloseScope}>
            ...
            <ModalContent>
                <ModalHeader >
                    <Box onClick={(e) => {e.stopPropagation(); onOpenTrainingDate();}} _hover={{cursor: 'default'}}>
                        Confirm Enrollment
                    </Box>
                </ModalHeader>
                <ModalCloseButton />
        ...
        <Modal isOpen={isOpenTrainingDate} onClose={onCloseTrainingDate} scrollBehavior='inside' size='2xl'>
            <ModalOverlay />
            <ModalContent>
                <ModalHeader>Training Record Entry</ModalHeader>
                <ModalCloseButton/>
                <ModalBody>
                    <VStack spacing={4}>
                        <FormControl>
                            <FormLabel>{`Start Date: (MMM DD YYYY)`}</FormLabel>
                            <Input type='text' value={startDate} onChange={(e) => setStartDate(e.target.value)} />
                        </FormControl>
                        <FormControl>
                            <FormLabel>{`End Date: (MMM DD YYYY)`}</FormLabel>
                            <Input type='text' value={endDate} onChange={(e) => setEndDate(e.target.value)} />
                        </FormControl>
                    </VStack>
                </ModalBody>
                <ModalFooter>
                    <Button w='100%' mr='3' colorScheme='yellow' onClick={() => handleNonAppearance(training_id, registration_id, trainee_id, 1, t_accountType)}>Non-Appearance</Button>
                    <Button w='100%' colorScheme='blue' bgColor='blue.700' onClick={() => handleInsertActualDates(startDate, endDate, training_id, registration_id, trainee_id, 1, t_accountType)} >Save Training Date</Button>
                </ModalFooter>
            </ModalContent>
        </Modal>
